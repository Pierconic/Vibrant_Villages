...
public class MobilityDescriptionProcedure {
	public static String execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (ExceptionalMobilityCheckProcedure.execute(entity)) {
			return (Component.translatable("deed_box.mobility.exceptional").getString()).replace("Testificate", villager_name);
		} else if (NeutralMobilityCheckProcedure.execute(entity)) {
			return (Component.translatable("deed_box.mobility.good").getString()).replace("Testificate", villager_name);
		} else if (TerribleMobilityCheckProcedure.execute(entity)) {
			return (Component.translatable("deed_box.mobility.bad").getString()).replace("Testificate", villager_name);
		}
		return "Error";
	}
}
